...
def AISString2Bits(name,length=20):
    ...
    if len(name)<length: name = name+'@'*int(length-len(name))
...
class AISStaticAndVoyageReportMessage(AISMessage):
    def __init__(self, id=5, repeat=0, mmsi=0, ais_version=0, imo=0, callsign=0, shipname=0,
        ...
        super(AISStaticAndVoyageReportMessage, self).__init__({
                    ...
                    'imo'          : ["uint", 30, imo], 
                    'callsign'     : ["uint", 42, AISString2Bits(callsign,length=42//6).uint if type(callsign) == str else callsign], 
                    'shipname'     : ["uint", 120, AISString2Bits(shipname,length=120//6).uint if type(shipname) == str else shipname], 
                    ...
                    'destination'  : ["uint", 120, AISString2Bits(destination,length=120//6).uint if type(destination) == str else destination], 
                    'dte'          : ["uint", 1, dte], 
                    'spare'        : ["uint", 1, spare]
                })
